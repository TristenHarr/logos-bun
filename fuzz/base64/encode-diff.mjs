...
if (OURS) {
  ...
  for (const s of strings) {
    ...
    const got = ours(s);
    if (got !== ref) fails.push(`base64(${JSON.stringify(s)}): ours=${JSON.stringify(got)} node=${JSON.stringify(ref)}`);
    checked++;
  }
  if (!fails.length) console.log(`PASS base64-encode: ${checked} strings agree with Node Buffer (seed ${seed})`);
}
