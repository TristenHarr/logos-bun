...
// a property; a[i] = v replaces an element. Value-semantic (the caller rebinds the
// same variable) — correct for mutate-then-use-same-variable, which is what these
// programs do; aliasing (p=o; p.x=1; o.x) needs reference semantics and is out of
// scope. Random build/update programs diffed vs Node (byte-exact via JSON.stringify).
