...
(nested grouping) + the COMPARISON tier (< > <= >= == === != !==) + the LOGICAL tier (&& || with short-circuit,
below comparisons) in pure LOGOS (recursion-threaded two-tier
evaluator, no parser stack). Exposed via `bun __js-eval`; differential-fuzzed vs Node's
own `eval()`: **~38k expressions (arith + nested parens + comparisons + && ||) across 18 seeds, 0 diffs** (fuzz/jsint/arith-diff.mjs). Full JS arithmetic+boolean expression grammar. This is the
