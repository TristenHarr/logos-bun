...
(nested grouping, via innermost-paren reduction) in pure LOGOS (recursion-threaded two-tier
evaluator, no parser stack). Exposed via `bun __js-eval`; differential-fuzzed vs Node's
own `eval()`: **~18k expressions (incl. deeply-nested parens) across 8 seeds, 0 diffs** (fuzz/jsint/arith-diff.mjs). This is the
