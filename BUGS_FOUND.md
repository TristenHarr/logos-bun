...
### BUG-40 · OURS · 2026-07-15 · correctness (open)
...
the object-spread fuzzer, which happened to name a variable the same as a key. **Status:** open —
`substitute` needs to skip a token in property position (right after `.`) and in key position (right
before ` : `), OR those passes must run before substitution. Worked around in the fuzzers by never
naming a variable the same as a key. **Tweet:** Found a gnarly one in my English-written JS engine:
